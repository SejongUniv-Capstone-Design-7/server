...
public interface DiagnosisResultRepository extends JpaRepository<DiagnosisResult, Long> {
    ...
    // 월별 지역별 병해충별 발생횟수
    @Query("select new capstone.capstone7.domain.diagnosis_result.dto.DiagnosisResultOfRegion(d.diseaseName, count(*)) from DiagnosisResult d where d.createdDate >= :startDate and d.createdDate < :endDate and d.region = :region group by d.diseaseName")
    List<DiagnosisResultOfRegion> findByRegion(@Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate, @Param("region") Region region);
